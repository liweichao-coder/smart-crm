...
- Connected AI order drafts into the Customer 360 workspace. The page can now generate a Copilot order draft from the account goal, preview line items and LLM notes, submit it through the real `/api/orders` endpoint, refresh the customer order list, and navigate to the order center.
- Improved Copilot order draft product selection. When products are not explicitly selected, the backend scores available inventory by business goal, customer industry, customer level, category, stock, and price instead of taking the first three catalog rows; no-stock product selections now return 400 before producing an unusable draft.
...
- Added a real-time customer health profile to the Customer 360 workspace. The backend now calculates health score, grade, trend, churn probability, factor breakdown, risk flags, strengths, and recommended actions from contacts, activities, leads, orders, cases, tasks, and Copilot recommendations rather than a static front-end label.
...
- Customer health profile targeted regression succeeded: workspace responses now keep the top-level customer health metric aligned with `health_profile.score`, expose six factor keys, and return risk flags, strengths, churn probability, and recommended actions from real CRM context.
...
- Targeted order-draft regression succeeded: explicit product selections are preserved, business goals recommend relevant private-deployment/permission/data products, and zero-stock selections are rejected.
