...
- `backend/` 已实现 FastAPI API、SQLite 数据库、资源创建/编辑/删除、订单创建、库存扣减、库存补货建议、库存流水、资源列表、Copilot 摘要和测试。
...
- 订单中心已接入 `/api/orders`、`/api/products` 和 `/api/inventory/*`，可查看订单筛选、订单明细、AI 标记、置信度、低库存预警、建议补货量和库存流水。
- 登录鉴权、商品/订单编辑、复杂报表、前后端统一鉴权仍未完成。
