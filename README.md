...
- `backend/` 已实现 FastAPI API、SQLite 数据库、资源创建/编辑/删除、订单创建、订单生命周期编辑、订单明细重算、库存差额调整、订单库存审计、库存补货建议、库存流水、资源列表、Copilot 摘要和测试。
...
- 客户、商品、订单和补货等写库动作会写入 `BusinessAuditLog` 审计表，可在操作审计页查看操作人、对象、状态、摘要和细节。
...
- 继续补分页、字段级校验和更多业务模块审计打点。
- 继续补分页、字段级校验、端到端冒烟测试和更细粒度权限审计。
- 将通用卡片、表格、看板拆为独立组件。
