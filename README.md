...
- `backend/` 已实现 FastAPI API、SQLite 数据库、团队成员管理、资源创建/编辑/删除、客户 360 工作台、客户互动记录、客户互动转任务、订单创建、订单生命周期编辑、订单明细重算、库存差额调整、订单审批流、订单审批策略拦截、订单库存审计、库存补货建议、库存流水、通知中心、销售 BI 报表、资源列表分页/搜索/筛选、服务端 RBAC、客户/业务 owner 数据范围、SQLite 轻量迁移、Copilot 摘要、CRM Skill 经营问答、推荐历史、推荐转任务和测试。
...
- 订单状态推进已接入真实审批策略：高金额、AI 低置信度、急交付、多明细或直接履约会触发经理复核；普通销售不能绕过审批直接确认高风险订单，经理/管理员通过审批后订单状态才会推进。
...
- 补端到端冒烟测试、更细粒度权限审计、多级审批 SLA 和更多跨表一致性校验。
- 将通用卡片、表格、看板拆为独立组件。
