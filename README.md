...
- `backend/` 已实现 FastAPI API、SQLite 数据库、资源创建/编辑/删除、订单创建、订单生命周期编辑、订单明细重算、库存差额调整、订单库存审计、库存补货建议、库存流水、资源列表分页/搜索/筛选、Copilot 摘要和测试。
...
- 服务端 RBAC 已覆盖主要业务 API：未登录请求返回 401，销售角色可维护客户但不能维护商品目录或读取审计；后续可继续增强字段级、数据范围级权限和审批策略。
