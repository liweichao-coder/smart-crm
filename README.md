...
1. 使用管理员账号登录。
2. 进入工作台，查看 KPI、销售管道和通知中心。
3. 打开客户页和客户 360，查看客户健康画像与互动时间线。
4. 进入线索/商机，看销售阶段流转和 L2C 主链路。
5. 打开 AI 副驾，展示评分理由、经营问答、推荐历史和转任务。
6. 进入智能录单，生成并复核订单草稿。
7. 打开订单页，确认订单落库、库存扣减和审批状态。
8. 打开销售报表，展示真实后端聚合和快照历史。

补充验收项：可再查看 AI 审计、业务审计、权限矩阵，并切换销售人员账号展示本人数据范围控制。
