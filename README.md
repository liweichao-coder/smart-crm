...
- AI 副驾页新增 CRM Skill 经营问答，前端调用 `/api/copilot/ask`，后端按当前用户数据范围聚合客户、互动、商机、订单、任务、工单和 Copilot 推荐，再调用 OpenAI-compatible LLM 生成回答；响应同时返回证据片段、下一步动作和推荐历史 ID，并写入 AI 审计与业务审计。
- Copilot 摘要、CRM Skill 经营问答和跟进话术都会写入 `CopilotRecommendation` 推荐历史表，可在 AI 副驾页查看客户、商机/问题、评分、下一步动作、话术草稿和 LLM/兜底状态；摘要评分会把客户健康分、流失概率、服务/关系/AI 执行因子和健康画像建议纳入可解释评分理由，并可在页面内展开查看。
