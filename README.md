...
- 智能录单草稿可在前端复核后提交到 `/api/orders`，生成真实订单并触发库存扣减；`/api/vision-extract` 会在后端根据客户/商品目录返回 `customer_id` 和条目级 `product_id`，并写入 `CaptureDraft` 草稿历史。前端提交订单时优先使用后端匹配 ID，提交成功后把草稿标记为 `submitted`，也支持按草稿/已提交/已作废筛选和人工作废草稿，避免只靠名称二次模糊匹配、丢失未提交草稿或重复提交已关闭草稿。
