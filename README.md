...
- AI Capture 智能录单页
- Orders 订单中心页
- 后端健康检查、认证登录注册、团队成员、商品、客户、联系人、客户互动、商机、工单、任务、目标、订单、订单审批、跨表一致性巡检和 AI 订单草稿接口
...
- 销售报表已接入 `/api/reports/sales-performance` 和 `/api/reports/approval-performance`，按真实订单、商机和审批记录聚合收入趋势、负责人/区域绩效、销售漏斗、AI 收入影响、库存风险、审批 SLA、风险等级和审批人工作量，支持负责人、区域和日期范围筛选。
- `backend/` 已实现 FastAPI API、SQLite 数据库、团队成员管理、资源创建/编辑/删除、客户 360 工作台、客户互动记录、客户互动转任务、订单创建、订单生命周期编辑、订单明细重算、库存差额调整、订单审批流、审批风险分级与 SLA、审批催办/转派、订单审批策略拦截、订单库存审计、库存补货建议、库存流水、通知中心、销售 BI 报表、跨表一致性巡检、资源列表分页/搜索/筛选、服务端 RBAC、客户/业务 owner 数据范围、SQLite 轻量迁移、Copilot 摘要、CRM Skill 经营问答、推荐历史、推荐转任务和测试。
...
- AI 副驾、智能录单和订单草稿接口会写入 `AIInteractionLog` 审计表，可在 AI 审计页查看模型、状态、耗时和摘要；AI 审计页同时接入 `/api/reports/ai-quality`，按真实日志展示 LLM 成功率、兜底率、场景覆盖、模型耗时和推荐转任务率。
...
- 补端到端冒烟测试、更细粒度权限审计、更多视图偏好保存和更多经营 BI 维度。
- 将通用卡片、表格、看板拆为独立组件。
